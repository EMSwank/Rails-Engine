class Api::V1::Items::SearchController < ApplicationController
  def show
    ...
    render json: Item.find_by_unit_price((params[:unit_price].to_d * 100).to_i) if params[:unit_price]
    render json: Item.find_by_merchant_id(params[:merchant_id]) if params[:merchant_id]
    render json: Item.order(:id).find_by_created_at(params[:created_at]) if params[:created_at]
    render json: Item.order(:id).find_by_updated_at(params[:updated_at]) if params[:updated_at]
  end
